...
const OHSA_R=[{region:"Foot & Ankle",oa:[["gastroc_soleus","Gastroc/Soleus"],["peroneals","Peroneals"],["tfl","TFL"]],ua:[["tibialis_anterior","Tib Anterior"],["glute_med","Glute Med"],["vmq_quads","VMO/Quads"]]},{region:"Knee",oa:[["adductors","Adductors"],["tfl","TFL"],["bicep_femoris","Bicep Femoris"]],ua:[["glute_med","Glute Med"],["vmq_quads","VMO/Quads"],["hamstrings","Hamstrings"]]},{region:"LPHC",oa:[["hip_flexors","Hip Flexors"],["piriformis","Piriformis"],["ql","QL"],["bicep_femoris","Bicep Femoris"]],ua:[["glute_max","Glute Max"],["tva_core","TVA/Core"],["hamstrings","Hamstrings"]]},{region:"Shoulder / Thoracic",oa:[["pec_minor","Pec Minor"],["lats","Lats"],["upper_trap","Upper Trap"],["levator_scap","Levator Scap"]],ua:[["lower_trap","Lower Trap"],["serratus_anterior","Serratus"],["deep_cervical_flexors","Deep Cervical"]]},{region:"Head & Neck",oa:[["sternocleidomastoid","SCM"],["upper_trap","Upper Trap"],["levator_scap","Levator Scap"]],ua:[["deep_cervical_flexors","Deep Cervical"],["lower_trap","Lower Trap"]]}];
const ML_T=[{k:"pec_minor",l:"Pec Minor"},{k:"lats",l:"Lats"},{k:"upper_trap",l:"Upper Trap"},{k:"levator_scap",l:"Levator Scap"},{k:"hip_flexors",l:"Hip Flexors"},{k:"tfl_it_band",l:"TFL / IT Band"},{k:"piriformis",l:"Piriformis"},{k:"adductors",l:"Adductors"},{k:"hamstrings",l:"Hamstrings"},{k:"gastroc_soleus",l:"Gastroc/Soleus"},{k:"ql",l:"QL"}];
const ST_T=[{k:"glute_max",l:"Glute Max"},{k:"glute_med",l:"Glute Med"},{k:"tva",l:"TVA/Core"},{k:"lower_trap",l:"Lower Trap"},{k:"serratus",l:"Serratus"},{k:"deep_cervical",l:"Deep Cervical"},{k:"hip_abductors",l:"Hip Abductors"},{k:"hamstrings_strength",l:"Hamstrings"},{k:"tibialis_anterior",l:"Tib Anterior"}];
  ...
  return h("div",{style:{paddingBottom:24}},
    ...
    sec==="postural"&&h(Card,null,h(CardH,{t:"POSTURAL SCREEN"}),h(CardB,null,
      h(G2,null,
        ...
        h(Fld,{label:"FOOT POSITION"},h(Sel,{value:form.postural.foot_position,onChange:v=>sp("postural.foot_position",v),options:[["neutral","Neutral"],["pronated_bilateral","Pronated — Bilateral"],["pronated_left","Pronated — Left"],["pronated_right","Pronated — Right"],["supinated","Supinated"]]})),
    ...
    sec==="ohsa"&&h(Card,null,h(CardH,{t:"OVERHEAD SQUAT — 5 REGION BREAKDOWN"}),h(CardB,{p:10},
      h("div",{style:{fontSize:11,color:C.gray,fontStyle:"italic",marginBottom:10}},"Tap to check. Overactive = tight/compensating. Underactive = weak/inhibited."),
      OHSA_R.map((region,ri)=>h("div",{key:ri,style:{marginBottom:12,background:C.grayLight,borderRadius:8,padding:10}},
        h("div",{style:{fontWeight:"bold",color:C.navy,fontSize:12,marginBottom:8}},region.region.toUpperCase()),
        h(G2,null,
          h("div",null,h("div",{style:{fontSize:10,fontWeight:"bold",color:C.red,marginBottom:5}},"OVERACTIVE"),region.oa.map(([key,label])=>h(CI,{key,checked:form.ohsa.overactive.includes(key),onClick:()=>togO("overactive",key),label,color:C.red}))),
          h("div",null,h("div",{style:{fontSize:10,fontWeight:"bold",color:C.green,marginBottom:5}},"UNDERACTIVE"),region.ua.map(([key,label])=>h(CI,{key,checked:form.ohsa.underactive.includes(key),onClick:()=>togO("underactive",key),label,color:C.green})))
        )
      )),
      h(Fld,{label:"OHSA NOTES",mb:0},h(TA,{value:form.ohsa.notes,onChange:v=>sp("ohsa.notes",v),placeholder:"Compensation patterns, squat depth...",rows:2}))
    )),
    ...
    sec==="muscle"&&h(Card,null,h(CardH,{t:"MUSCLE LENGTH TESTS"}),h(CardB,{p:10},
      h("div",{style:{fontSize:11,color:C.gray,fontStyle:"italic",marginBottom:10}},"Tap result for each muscle."),
      ML_T.map(({k,l})=>h("div",{key:k,style:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 10px",borderBottom:`1px solid ${C.grayBorder}`,gap:8,flexWrap:"wrap"}},
        h("div",{style:{fontSize:13,color:C.navy,fontWeight:"bold",minWidth:120}},l),
        h("div",{style:{display:"flex",gap:5}},
          [["tight","Tight",C.red],["normal","Normal",C.green],["hypermobile","Hyper",C.amber],["not_tested","—",C.gray]].map(([v,lbl,color])=>h("div",{key:v,onClick:()=>sp(`muscleLength.${k}`,v),style:{padding:"4px 9px",borderRadius:6,cursor:"pointer",fontSize:11,fontWeight:"bold",border:`1.5px solid ${(form.muscleLength[k]||"not_tested")===v?color:C.grayBorder}`,background:(form.muscleLength[k]||"not_tested")===v?color+"22":C.white,color:(form.muscleLength[k]||"not_tested")===v?color:C.gray}},lbl))
        )
      ))
    )),
    sec==="strength"&&h(Card,null,h(CardH,{t:"MUSCLE STRENGTH TESTS"}),h(CardB,{p:10},
      h("div",{style:{fontSize:11,color:C.gray,fontStyle:"italic",marginBottom:10}},"Tap result for each muscle."),
      ST_T.map(({k,l})=>h("div",{key:k,style:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 10px",borderBottom:`1px solid ${C.grayBorder}`,gap:8,flexWrap:"wrap"}},
        h("div",{style:{fontSize:13,color:C.navy,fontWeight:"bold",minWidth:130}},l),
        h("div",{style:{display:"flex",gap:5}},
          [["normal","Normal",C.green],["weak","Weak",C.amber],["inhibited","Inhibited",C.red],["not_tested","—",C.gray]].map(([v,lbl,color])=>h("div",{key:v,onClick:()=>sp(`strengthTests.${k}`,v),style:{padding:"4px 9px",borderRadius:6,cursor:"pointer",fontSize:11,fontWeight:"bold",border:`1.5px solid ${(form.strengthTests[k]||"not_tested")===v?color:C.grayBorder}`,background:(form.strengthTests[k]||"not_tested")===v?color+"22":C.white,color:(form.strengthTests[k]||"not_tested")===v?color:C.gray}},lbl))
        )
      ))
    )),
...
function NutritionView({client}){
  ...
  const adj=(type,pct)=>{const oth=["protein","carbs","fat"].filter(t=>t!==type);const rem=100-pct;const tot=macros[oth[0]].pct+macros[oth[1]].pct;const r0=tot>0?macros[oth[0]].pct/tot:0.5,r1=tot>0?macros[oth[1]].pct/tot:0.5;const nm={...macros,[type]:{pct,grams:Math.round((calories*pct/100)/(type==="fat"?9:4))},[oth[0]]:{pct:Math.round(rem*r0),grams:Math.round((calories*rem*r0/100)/(oth[0]==="fat"?9:4))},[oth[1]]:{pct:Math.round(rem*r1),grams:Math.round((calories*rem*r1/100)/(oth[1]==="fat"?9:4))}};setMacros(nm);LS.set(`tbf_macros_${client.id}`,nm);};
  const calc=()=>{const w=parseFloat(cf.weight),ht=parseFloat(cf.height),a=parseFloat(cf.age);if(!w||!ht||!a) return;const ACT={sedentary:1.2,light:1.375,moderate:1.55,active:1.725,very:1.9};const ADJ={loss:-500,maintenance:0,gain:350};const bmr=cf.sex==="male"?10*w+6.25*ht-5*a+5:10*w+6.25*ht-5*a-161;const target=Math.round(bmr*ACT[cf.activity])+ADJ[cf.goal];setCr({calories:target,protein:{pct:32,grams:Math.round((target*0.32)/4)},carbs:{pct:43,grams:Math.round((target*0.43)/4)},fat:{pct:25,grams:Math.round((target*0.25)/9)}});};
  const applyCalc=()=>{if(!cr) return;setCalories(cr.calories);setMacros(cr);LS.set(`tbf_cals_${client.id}`,cr.calories);LS.set(`tbf_macros_${client.id}`,cr);setShowCalc(false);setCr(null);};
  const MB=({label,pct,grams,color,type})=>h("div",{style:{marginBottom:16}},h("div",{style:{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}},h("span",{style:{fontWeight:"bold",color:C.navy}},label),h("span",{style:{color,fontWeight:"bold"}},`${pct}% — ${grams}g`)),h("input",{type:"range",min:10,max:70,value:pct,onChange:e=>adj(type,Number(e.target.value)),style:{width:"100%",accentColor:color}}),h("div",{style:{background:C.grayLight,borderRadius:6,height:8,marginTop:4}},h("div",{style:{background:color,width:`${pct}%`,height:8,borderRadius:6}})));
  const meals=client.nutrition?.meals||[{label:"Breakfast",foods:["Eggs","Spinach","Avocado"],note:"Protein + healthy fats."},{label:"Lunch",foods:["Chicken breast","Brown rice","Broccoli"],note:"Balanced meal."},{label:"Dinner",foods:["Salmon","Sweet potato","Asparagus"],note:"Omega-3s + slow carbs."},{label:"Snack",foods:["Greek yogurt","Blueberries"],note:"Protein + antioxidants."}];
  return h("div",{style:{paddingBottom:24}},
    h(Btn,{onClick:()=>setShowCalc(!showCalc),color:showCalc?C.gray:C.teal,full:true,st:{marginBottom:12}},showCalc?"Hide Calculator":"📊 Calculate My Calories & Macros"),
    showCalc&&h(Card,null,h(CardH,{t:"CALORIE & MACRO CALCULATOR"}),h(CardB,null,
      h(G2,null,h(Fld,{label:"WEIGHT (kg)"},h(Inp,{value:cf.weight,onChange:v=>setCf(p=>({...p,weight:v})),placeholder:"e.g. 80"})),h(Fld,{label:"HEIGHT (cm)"},h(Inp,{value:cf.height,onChange:v=>setCf(p=>({...p,height:v})),placeholder:"e.g. 175"})),h(Fld,{label:"AGE"},h(Inp,{value:cf.age,onChange:v=>setCf(p=>({...p,age:v})),placeholder:"e.g. 32"})),h(Fld,{label:"SEX"},h(Sel,{value:cf.sex,onChange:v=>setCf(p=>({...p,sex:v})),options:[["male","Male"],["female","Female"]]}))),
      h(Fld,{label:"ACTIVITY LEVEL"},h(Sel,{value:cf.activity,onChange:v=>setCf(p=>({...p,activity:v})),options:[["sedentary","Sedentary (desk job, no exercise)"],["light","Light (1–2 days/week)"],["moderate","Moderate (3–4 days/week)"],["active","Active (5–6 days/week)"],["very","Very Active (athlete/physical job)"]]})),
      h(Fld,{label:"GOAL"},h("div",{style:{display:"flex",gap:8}},[["loss","Weight Loss",C.red],["maintenance","Maintenance",C.teal],["gain","Muscle Gain",C.green]].map(([val,label,color])=>h("div",{key:val,onClick:()=>setCf(p=>({...p,goal:val})),style:{flex:1,padding:"8px 4px",borderRadius:7,border:`2px solid ${cf.goal===val?color:C.grayBorder}`,background:cf.goal===val?color+"15":C.white,textAlign:"center",cursor:"pointer",fontSize:11,fontWeight:"bold",color:cf.goal===val?color:C.gray}},label)))),
      h(Btn,{onClick:calc,color:C.teal,full:true},"Calculate My Targets"),
      cr&&h("div",{style:{marginTop:14,padding:12,background:C.tealLight,borderRadius:8}},
        h("div",{style:{fontWeight:"bold",color:C.navy,fontSize:15,marginBottom:8,textAlign:"center"}},`${cr.calories.toLocaleString()} calories / day`),
        h("div",{style:{display:"flex",gap:8,marginBottom:10}},[["Protein",cr.protein.grams+"g",C.teal],["Carbs",cr.carbs.grams+"g",C.amber],["Fat",cr.fat.grams+"g",C.red]].map(([l,g,color])=>h("div",{key:l,style:{flex:1,textAlign:"center",padding:8,background:C.white,borderRadius:6,border:`1px solid ${color}33`}},h("div",{style:{fontSize:16,fontWeight:"bold",color}},g),h("div",{style:{fontSize:10,color:C.gray}},l)))),
    ...
    h(Card,null,h(CardH,{t:`DAILY TARGET — ${calories.toLocaleString()} CAL`}),h(CardB,null,
      ...
      h("div",{style:{fontSize:10,color:C.gray,textAlign:"center",fontStyle:"italic"}},"Drag sliders to adjust — others auto-balance to 100%")
    )),
    meals.map((meal,mi)=>{const foods=mW[mi]||meal.foods,search=mS[mi]||"";const results=search.length>1?ALL_FOODS.filter(f=>f.name.toLowerCase().includes(search.toLowerCase())).slice(0,8):[];return h(Card,{key:mi},h(CardH,{t:meal.label.toUpperCase()}),h(CardB,null,h("div",{style:{fontSize:11,color:C.gray,fontStyle:"italic",marginBottom:8}},meal.note),foods.map((food,fi)=>h("div",{key:fi,style:{display:"flex",gap:8,padding:"4px 0",borderBottom:fi<foods.length-1?`1px solid ${C.grayBorder}`:"none"}},h("span",{style:{color:C.teal,fontWeight:"bold"}},"•"),h("span",{style:{fontSize:13,color:C.navy}},food))),h("div",{style:{marginTop:10}},h(Inp,{value:search,onChange:v=>setMS(p=>({...p,[mi]:v})),placeholder:"Search whole food library...",st:{fontSize:12}}),results.length>0&&h("div",{style:{border:`1px solid ${C.grayBorder}`,borderRadius:6,overflow:"hidden",marginTop:4}},results.map((f,i)=>h("div",{key:i,onClick:()=>{setMW(p=>({...p,[mi]:[...foods,f.name]}));setMS(p=>({...p,[mi]:""}));},style:{padding:"6px 10px",cursor:"pointer",fontSize:12,color:C.navy,background:i%2===0?C.white:C.grayLight,display:"flex",justifyContent:"space-between"}},h("span",null,f.name),h(Tag,{label:f.cat,color:C.teal})))))));
    }),
    client.nutrition?.supplements&&h(Card,null,h(CardH,{t:"SUPPLEMENT TIMING",color:C.teal}),h(CardB,null,client.nutrition.supplements.map((s,i)=>h("div",{key:i,style:{marginBottom:12}},h("div",{style:{fontWeight:"bold",color:C.navy,fontSize:12,marginBottom:5}},s.timing),h("div",{style:{display:"flex",flexWrap:"wrap"}},s.items.map((item,j)=>h(Pill,{key:j,label:item,color:C.teal})))))))
  );
}
...
function AddClientForm({onAdd,onClose}){
  const [form,setForm]=useState({name:"",email:"",password:"",goal:"posture",phase:"1",focus:"",restrictions:""});
  const f=(k,v)=>setForm(p=>({...p,[k]:v}));
  const handleAdd=()=>{if(!form.name||!form.email||!form.password) return;const tmpl=TEMPLATES[form.goal];onAdd({id:form.email.toLowerCase().replace(/[^a-z0-9]/g,"_"),name:form.name,email:form.email,password:form.password,role:"client",phase:parseInt(form.phase),focus:form.focus||tmpl.focus,restrictions:form.restrictions?form.restrictions.split(",").map(r=>r.trim()):[],days:[],schedule:[],nutrition:null});};
  return h("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}},
    h("div",{style:{background:C.white,borderRadius:14,width:"100%",maxWidth:460,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.35)"}},
      h("div",{style:{background:C.navy,color:C.white,padding:"16px 20px",borderRadius:"14px 14px 0 0",display:"flex",justifyContent:"space-between",alignItems:"center"}},h("div",{style:{fontWeight:"bold",fontSize:15}},"Add New Client"),h("button",{onClick:onClose,style:{background:"none",border:"none",color:C.white,fontSize:22,cursor:"pointer",lineHeight:1}},"×")),
      h("div",{style:{padding:20}},
        ...[["name","FULL NAME","Jane Smith","text"],["email","EMAIL","jane@email.com","email"],["password","INITIAL PASSWORD","Temporary password","password"]].map(([k,label,ph,type])=>h(Fld,{key:k,label},h(Inp,{value:form[k],onChange:v=>f(k,v),placeholder:ph,type}))),
        ...
        h(Fld,{label:"STARTING PHASE"},h(Sel,{value:form.phase,onChange:v=>f("phase",v),options:[["1","Phase 1"],["2","Phase 2"],["3","Phase 3"]]})),
        h(Fld,{label:"FOCUS (optional)"},h(Inp,{value:form.focus,onChange:v=>f("focus",v),placeholder:"e.g. UCS | Lower Back | Post-Op"})),
        h(Fld,{label:"RESTRICTIONS (comma separated)"},h(Inp,{value:form.restrictions,onChange:v=>f("restrictions",v),placeholder:"e.g. No overhead pressing, No lumbar flexion"})),
        h("div",{style:{display:"flex",gap:10,marginTop:16}},h(Btn,{onClick:onClose,color:C.grayLight,fg:C.navy,full:true},"Cancel"),h(Btn,{onClick:handleAdd,color:C.teal,full:true},"Add Client"))
      )
    )
  );
}
